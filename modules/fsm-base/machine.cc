...
namespace fsm {
    ...
    bool StateMachine::ProcessNormalStateTransition(const EventSharedPtr &event) {
        if (current_state_) {
            ...
            for (auto i = current_state_->transitions_.begin(); i != current_state_->transitions_.end(); ++i) {
                ...
                if (transition->IsMatch(event, __SELF__)) {
                    ...
                    if (transition->transition_type() == Transition::TransitionType::kNormalTransition) {
                        ...
                        try {
                            current_state_->OnExit(__SELF__, current_state_);
                    ...
                    try {
                        transition->OnTransition(__SELF__,
                                                 current_state_,
                    ...
                    if (transition->transition_type() == Transition::TransitionType::kNormalTransition) {
                        {
                            ...
                            if (machine_set) {
                                machine_set->UpdateTimeoutMachine(__SELF__, current_state_->timeout());
                            ...
                            try {
                                current_state_->OnEnter(__SELF__, current_state_);
            ...
            for (auto i = meta_state_->transitions_.begin();
                ...
                if (transition->IsMatch(event, __SELF__)) {
                    ...
                    if (transition->transition_type() == Transition::TransitionType::kNormalTransition) {
                        ...
                        if (machine_set &&
                            to_status) {
                            machine_set->UpdateTimeoutMachine(__SELF__, to_status->timeout());
    ...
    bool ActionMachine::Process(EventSharedPtr event) {
        ...
        for (const auto &non_transitive_transition: non_transitive_actions_) {
            if (non_transitive_transition->IsMatch(event, __SELF__)) {
                LOG(INFO) << "Non-transitive action: " << non_transitive_transition->name()
                          << " in "
                          << " (" << type_.name() << " " << name_ << ").";

                try {
                    non_transitive_transition->OnAction(__SELF__,
